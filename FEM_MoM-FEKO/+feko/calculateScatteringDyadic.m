...
% 
% Inputs:
%   model     ~ filename of CFM file with the FEKO model and settings. it
%               is expected that it is saved in [models] folder
...
tempFileName = 'calculateScatteringDyadics_TEMP';
...
for thisBatch = 1:length(freqA)
    ...
    for thisWave = 1:nWaves
        ...
        feko.preFEKO_1PW_nFF_Sdyadic(preFEKOfileName, ...
            model, Materials, batchFreq, PW(thisWave, :), FF, EXTR);
        ...
        if sizeFits
            S(:, thisWave, freqA(thisBatch):freqB(thisBatch)) = thisColumn;
        end
