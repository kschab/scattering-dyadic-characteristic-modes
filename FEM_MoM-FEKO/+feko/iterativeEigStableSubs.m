function [Q, D, Sn, nIter, tn, Err, err] = iterativeEigStableSubs(...
    Afun, W, C, nPW, nModes, relError)
...
% C        - permutation matrix for scattering dyadic matrices
...
while err(end) > relError
    ...
    fn1 = Afun(an, 1); % scattered field computed with FEKO
    fn2 = C*(conj(Afun(C*(conj(2*fn1 + an)), 2))); % adjoint version for passive obj.
